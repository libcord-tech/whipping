...
class Whipping(commands.Cog):
    ...
    def _stripe_users(self, uc_members: List[int], libcord_members: List[int], stripe_count: int = 3) -> Dict[int, List[int]]:
    ...
    async def setup_assignments(self, ctx: commands.Context, stripe_count: int = 3):
        ...
        libcord_members = [m.id for m in guild.members if not m.bot and uc_role not in m.roles and (not jc_role or jc_role not in m.roles)]
    ...
    @commands.is_owner()
    async def reassign_user(self, ctx: commands.Context, user: discord.Member, from_uc: discord.Member, to_uc: discord.Member):
